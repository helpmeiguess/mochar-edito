...
func _on_note5_button_down():
	for i in $'/root/Node2D'.selected:
		if $'/root/Node2D'.keyframes[i-1][1] == 5:
...
func _on_note5_button_up():
	for i in $'/root/Node2D'.selected:

		$'/root/Node2D'.keyframes[i-1][3] = (rect_position.x - $'/root/Node2D'.ogposx[ 5-1]) / 0.35
		$'/root/Node2D'.keyframes[i-1][4] = (rect_position.y - $'/root/Node2D'.ogposy[ 5-1]) / 0.35
