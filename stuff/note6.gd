...
func _on_note6_button_down():
	for i in $'/root/Node2D'.selected:
		if $'/root/Node2D'.keyframes[i-1][1] == 6:
...
func _on_note6_button_up():
	for i in $'/root/Node2D'.selected:

		$'/root/Node2D'.keyframes[i-1][3] = (rect_position.x - $'/root/Node2D'.ogposx[ 6-1]) / 0.35
		$'/root/Node2D'.keyframes[i-1][4] = (rect_position.y - $'/root/Node2D'.ogposy[ 6-1]) / 0.35
